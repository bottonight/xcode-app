...
实体按键按下后，固件会先向 `4348411D-...` 发送 Start Scan Notify。App 在空闲状态
只要收到这条特征通知（不解析首字节）就立即显示「扫描中」。随后同一特征再发
`0xFF + 4 字节 scan index` 完成包，App 才通过 `43484127-...` 拉光谱并进入「处理数据中」。
...
实体按键按下后，固件会先向 Notify 推送第 10 条按键秒回（地址 `0x4000300A`，示例
`55 D5 DD C4 00 09 40 00 30 0A 00 24`）。App 在空闲状态收到该帧后立即显示「扫描中」，
随后按第 5 条（地址 `0x40003005`）接收 64 组测量分包并进入「处理数据中」。APP 主动扫描
仍写 `0x40003005` 读命令 `[0x01, 0x00]`，不经过按键秒回。
