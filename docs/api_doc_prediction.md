...
`data` 共 3822 个 0–255 字节。现有后端 `DataDecoder` 要求 JSON 中编码为十进制
数字字符串（例如 `"12"`）；多次 N 次则把 N 段 3822 点
**首尾拼成一条一维数组**，长度 `3822 * N`。
...
`/Prediction` 的 `builtin` 与 `/SetReference` 的 `data` 都是设备返回的
**原始序列化字节流**，固定长度 **3822**。客户端仅移除 BLE 多包的首字节包序号，
不在本地解码为 `{w, i}`。
...
`builtin` 共 3822 个 0–255 字节，并按十进制数字字符串编码；使用内置参考时
`is_builtin=true`。
...
- `/SaveWaveTI` 的 `wave`：与 NIR 原始扫描同结构（3822 点十进制数字字符串）。单次一维；多次为二维（每条 3822），**不是**首尾拼接。
...
- **描述**: 设置/更新设备参考光谱（校准）
- **参数** (json):
  - data: 参考光谱。普通设备为单次 NIR 原始字节流（长度 3822，按十进制数字字符串编码，见文首「扫描数据格式」）；`is_qt=true` 时直接作为 `{w, i}` 写入
  - mac_NIR
